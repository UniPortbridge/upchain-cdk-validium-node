...
func (z *ZKEVMEndpoints) GetReceiptsByBlockNumber(number types.BlockNumber) (interface{}, types.Error) {
	return z.txMan.NewDbTxScope(z.state, func(ctx context.Context, dbTx pgx.Tx) (interface{}, types.Error) {
		if number == types.PendingBlockNumber {
			lastBlock, err := z.state.GetLastL2Block(ctx, dbTx)
			if err != nil {
			...
			header.UncleHash = ethTypes.EmptyUncleHash
			block := ethTypes.NewBlockWithHeader(header)
			rpcBlock, err := types.NewBlock(block, nil, true, true)
			if err != nil {
				return RPCErrorResponse(types.DefaultErrorCode, "couldn't build the pending block response", err, true)
			}
		...
		rpcBlock, err := types.NewBlock(block, receipts, true, true)
		if err != nil {
			return RPCErrorResponse(types.DefaultErrorCode, fmt.Sprintf("couldn't build block response for block by number %v", blockNumber), err, true)
		}

		blockReceipts := make([]types.BlockReceipt, 0, len(receipts))
		...
		for _, tx := range rpcBlock.Transactions {
			blockReceipt := &types.BlockReceipt{
				...
				TxHash:            tx.Tx.Hash,
				TxIndex:           tx.Tx.TxIndex,
				From:              tx.Tx.Receipt.FromAddr,
				EffectiveGasPrice: tx.Tx.Receipt.EffectiveGasPrice,
				GasUsed:           tx.Tx.Receipt.GasUsed,
				Status:            tx.Tx.Receipt.Status,
			}
